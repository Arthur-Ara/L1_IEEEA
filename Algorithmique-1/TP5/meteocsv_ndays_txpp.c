//  Utilisation :
//    meteocsv_ndays_txpp [FILE]...
//
//  Sans paramètre, rappelle la syntaxe.
//  Sinon, affiche le nombre de joueur de forte chaleur
...
//  ndays_txpp : Affiche le nombre de forte chaleur
...
const char *ndays_txpp(const char *filename, float *msrptr);

#define MSR_FORMAT  "%.0f tx"
...
int main(int argc, char *argv[]) {
  ...
  for (int k = 1; k < argc; ++k) {
    float msr;
    const char *r = ndays_txpp(argv[k], &msr);
    if (r != NULL) {
...
const char *ndays_txpp(const char *filename, float *msrptr) {
  ...
  //    && s == (somme du champ tx>=30 des relevés lus,
